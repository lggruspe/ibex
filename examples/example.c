...
{
    struct rnd_expr *nonzero = rnd_symbol('1', '9');
    struct rnd_expr *digit = rnd_symbol('0', '9');
    struct rnd_expr *zero = rnd_symbol('0', '0');
...
{
    struct rnd_expr *digit = rnd_symbol('0', '9');
    struct rnd_expr *dot = rnd_symbol('.', '.');
    struct rnd_expr *decimal = rnd_concatenation(dot, 
            rnd_concatenation(digit, rnd_closure(digit)));
    struct rnd_expr *eE = rnd_union(rnd_symbol('e', 'e'), rnd_symbol('E', 'E'));
    struct rnd_expr *sign = optional(rnd_union(rnd_symbol('-', '-'), rnd_symbol('+', '+')));
...
{
    struct rnd_expr *underscore = rnd_symbol('_', '_');
    struct rnd_expr *small = rnd_symbol('a', 'z');
    struct rnd_expr *big = rnd_symbol('A', 'Z');
...
{
    struct rnd_expr *space = rnd_symbol(' ', ' ');
    struct rnd_expr *tab = rnd_symbol('\t', '\t');
...
{
    struct rnd_expr *backslash = rnd_symbol('\\', '\\');
    struct rnd_expr *quote = rnd_symbol('\'', '\'');
    struct rnd_expr *t = rnd_symbol('t', 't');
    struct rnd_expr *n = rnd_symbol('n', 'n');
...
{
    struct rnd_expr *quotes = rnd_symbol('"', '"');
    struct rnd_expr *backslash = rnd_symbol('\\', '\\');
